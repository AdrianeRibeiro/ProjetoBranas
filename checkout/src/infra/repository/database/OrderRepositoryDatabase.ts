...
export default class OrderRepositoryDatabase implements OrderRepository {
  ...
  async save(order: Order): Promise<void> {
    const [orderData] = await this.connection.query("insert into orders (code, cpf, issue_date, sequence, freight, coupon_code, coupon_percentage, total) values ($1, $2, $3, $4, $5, $6, $7, $8) returning *", [order.getCode(), order.cpf.value, order.date, order.sequence, order.freight, order.coupon?.code, order.coupon?.percentage, order.getTotal()]);
  ...
  async getByCpf(cpf: string): Promise<Order[]> {
    const ordersData = await this.connection.query("select * from orders where cpf = $1", [cpf])
    const orders: Order[] = []
  ...
  async count(): Promise<number> {
    const [options] = await this.connection.query("select count(*)::integer as count from orders", [])
